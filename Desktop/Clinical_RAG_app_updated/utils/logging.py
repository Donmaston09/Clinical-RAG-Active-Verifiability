...
def _row_from_crts(query: str, crts: Dict) -> Dict:
    return {
        ...
        'crts': crts.get('crts'),
        'L': crts.get('L'),
        'alpha': crts.get('weights',{}).get('alpha'),
        'beta': crts.get('weights',{}).get('beta'),
        'gamma': crts.get('weights',{}).get('gamma'),
        'delta': crts.get('weights',{}).get('delta'),
    }
...
def log_crts_jsonl(query: str, crts: Dict, jsonl_path: str = 'crts_log.jsonl', extra: Optional[Dict] = None) -> None:
...
def log_crts_both(query: str, crts: Dict, csv_path: str = 'crts_log.csv', jsonl_path: str = 'crts_log.jsonl', extra: Optional[Dict] = None) -> None:
